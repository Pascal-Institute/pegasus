...
// Forward declaration for ImageLayer type
interface ImageLayer {
  extensionCombo?: HTMLElement;
...
export class GifAnimation {
  ...
  private metadata: GifMetadata | null;
  private originalBuffer: Buffer | null;
  private currentFrame: number;
  private isPlaying: boolean;
  private animationId: NodeJS.Timeout | null;
  ...
  private frameCounter: HTMLSpanElement | null;
   ...
   */
  initControls(): void {
    if (this.controlsContainer) return; // Already initialized
    ...
    if (this.layer.extensionCombo && this.layer.extensionCombo.parentNode) {
      this.layer.extensionCombo.parentNode.insertBefore(
        this.controlsContainer,
        this.layer.extensionCombo.nextSibling
      );
    }
  }
   ...
   */
  load(buffer: Buffer, metadata: sharp.Metadata): void {
  ...
  private scheduleNextFrame(): void {
    if (!this.isPlaying || !this.isAnimated()) return;
  ...
  private async displayFrame(frameIndex: number): Promise<void> {
    if (!this.isAnimated() || !this.originalBuffer) return;
    ...
    try {
      ...
      image.src = `data:image/png;base64,${frameResult.buffer.toString(
        "base64"
      )}`;
      image.onload = () => {
