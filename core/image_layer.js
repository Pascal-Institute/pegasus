...
class ImageLayer {
    ...
    async updatePreview(buffer, info, colors) {
        ...
        this.overlayCanvas.style.left = "0";
        this.overlayCanvas.style.top = "0";
        this.image.src = `data:image/${this.extension};base64,${buffer.toString("base64")}`;
        this.image.onload = () => {
        ...
        if (Array.isArray(extractedColors) && extractedColors.length > 0) {
            ...
            for (let i = extractedColors.length; i < this.colorBox.colors.length; i++) {
    ...
    restoreFromHistory(state) {
        ...
        this.image.src = `data:image/${state.extension};base64,${state.buffer.toString("base64")}`;
    ...
    async convertFormat(newExtension) {
        ...
        try {
            ...
            const result = await ImageProcessor.convertFormat(this.buffer, this.extension, newExtension);
            ...
            if (this.filepath) {
                this.filepath = ImageProcessor.updateFileExtension(this.filepath, newExtension);
            }
            this.updatePreview(result.buffer, result.info);
    ...
    async saveImage(filepath) {
        ...
        try {
            ...
            if (this.extension === "svg" && this.svgData) {
                ...
                const base64Data = this.image.src.replace(`data:image/${this.extension};base64,`, "");
    ...
    async applyCrop(cropData) {
        ...
        try {
            ...
            const result = await ImageProcessor.applyCrop(this.buffer, this.info, cropData);
            this.updatePreview(result.buffer, result.info);
        }
        catch (error) {
            ipcRenderer.send("showNotificationREQ", "imgkit-error");
...
//# sourceMappingURL=image_layer.js.map